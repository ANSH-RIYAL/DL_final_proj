...
#         # # # file_path = f'./../Dataset_Student/unlabeled/video_{i}/'
#         filepath = f'./../Dataset_Student/unlabeled/video_{i}/'
#         # obtain x values.
#         for j in range(num_hidden_frames):
#             x.append(torch.tensor(plt.imread(filepath + f'image_{j}.png')).permute(2, 0, 1))
#         x = torch.stack(x, 0)
...
best_model_path = './checkpoint_frame_prediction.pth' # load saved model to restart from previous best model (lowest val loss) checkpoint
...
#             optimizer.zero_grad()
            ...
            val_pbar.set_description('train loss: {:.4f}'.format(loss.item()))
